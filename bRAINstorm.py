...
class Raindrop:
    ...
    def is_off_screen(self, miss_y=RAINDROP_MISS_Y):
        return self.y > miss_y
...
class Game:
    ...
    def update_layout(self):
        input_y = SCREEN_HEIGHT - 280 if self.virtual_keyboard_enabled else SCREEN_HEIGHT - 86
        self.input_box = pygame.Rect(130, input_y, SCREEN_WIDTH - 260, 62)
    ...
    def get_raindrop_miss_y(self):
        # Keep the miss boundary locked to the Answer box so UI and gameplay stay in sync.
        if hasattr(self, "input_box"):
            return self.input_box.y - 20
    ...
    def update(self):
        difficulty_level = self.get_difficulty_level()
        self.speed = self.get_speed_for_level(difficulty_level)
        miss_y = self.get_raindrop_miss_y()

        for raindrop in self.raindrops[:]:
            raindrop.speed = self.speed
            raindrop.update()

            if not raindrop.is_off_screen(miss_y):
                continue
    ...
    def draw_menu(self):
        ...
        self.draw_text_with_shadow("bRAINstorm", self.font_large, WHITE, (SCREEN_WIDTH // 2, 150))
    ...
    def render_hud_value(self, text, color, max_width):
        ...
        fallback_font = pygame.font.SysFont("segoe ui", 20, bold=True)
        return fallback_font.render(text, True, color)

    def draw_hud(self):
        hud_rect = pygame.Rect(20, 16, SCREEN_WIDTH - 40, 88)
        ...
        content_rect = hud_rect.inflate(-18, -14)
        gap = 12
        section_width = (content_rect.width - 2 * gap) // 3
        lives_rect = pygame.Rect(content_rect.x, content_rect.y, section_width, content_rect.height)
        level_rect = pygame.Rect(lives_rect.right + gap, content_rect.y, section_width, content_rect.height)
        score_rect = pygame.Rect(level_rect.right + gap, content_rect.y, section_width, content_rect.height)

        for rect in (lives_rect, level_rect, score_rect):
        ...
        self.screen.blit(lives_label, lives_label.get_rect(midtop=(lives_rect.centerx, lives_rect.y + 6)))

        heart_spacing = 28
        first_heart_x = lives_rect.centerx - heart_spacing
        heart_y = lives_rect.centery + 7
        for index in range(3):
            color = RED if index < self.lives else (201, 215, 225)
            heart_x = first_heart_x + index * heart_spacing
            pygame.draw.circle(self.screen, color, (heart_x, heart_y), 9)
            pygame.draw.circle(self.screen, WHITE, (heart_x - 3, heart_y - 3), 3)

        level_label = self.font_tiny.render("LEVEL", True, label_color)
        self.screen.blit(level_label, level_label.get_rect(midtop=(level_rect.centerx, level_rect.y + 6)))
        level_value = self.render_hud_value(str(difficulty_level), value_color, level_rect.width - 22)
        self.screen.blit(level_value, level_value.get_rect(center=(level_rect.centerx, level_rect.centery + 9)))

        score_label = self.font_tiny.render("SCORE", True, label_color)
        self.screen.blit(score_label, score_label.get_rect(midtop=(score_rect.centerx, score_rect.y + 6)))
        score_value = self.render_hud_value(str(self.score), value_color, score_rect.width - 22)
        self.screen.blit(score_value, score_value.get_rect(center=(score_rect.centerx, score_rect.centery + 9)))
    ...
    def draw_miss_boundary(self):
        y = self.get_raindrop_miss_y()
        if y <= 90 or y >= SCREEN_HEIGHT - 20:
            return

        start_x = 24
        end_x = SCREEN_WIDTH - 24
        segment = 16
        gap = 10

        shadow_color = (90, 34, 34)
        line_color = (241, 108, 108)

        x = start_x
        while x < end_x:
            x2 = min(x + segment, end_x)
            pygame.draw.line(self.screen, shadow_color, (x, y + 1), (x2, y + 1), 2)
            pygame.draw.line(self.screen, line_color, (x, y), (x2, y), 2)
            x += segment + gap
    ...
    def draw_on_screen_keyboard(self):
        if not self.virtual_keyboard_enabled:
            return
